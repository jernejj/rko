...
public class ConnectionProcessor implements Runnable
{
	...
	else if(s.charAt(0) == 'Z' && s.charAt(1) == ' ')
	{
	    ...
	    if( tmp.vrniStanje(izdelek)-delta < 0 )
	    	return "Ni dovolj zaloge za izdelek "+ izdelek +" . Stanje skladisca je:"+tmp.vrniStanje();
	    else
	    	tmp.zmanjsaj(izdelek, delta);
	    return "Zmanjsaj " + izdelek +" za " + delta + " enot. Stanje skladišèa je:"+tmp.vrniStanje();
	   
	}
	else if(s.charAt(0) == 'K' && s.charAt(1) == ' ')
	    return "Prekinjam...";
	...
	try
	{
	    ...
	    while( true )
	    {
		if((response = processPosting(din.readUTF())) != null)
